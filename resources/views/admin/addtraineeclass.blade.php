...
<div class="row" style="    margin: 80px 30px; direction: rtl;">
    ...
    color: white;">
                            ...
                            <button onclick="togglePopuo()" class="bbtn">اضافة متدرب جديد</button>
                        </div>
    ...
    color: white;">
                            ...
                            <td>{{$i++}}</td>
                                <td>  {{$call->Bimar_Course_Enrollment->bimar_training_course->tr_course_name_ar}}  </td>
                                    ...
                                    <a href="{{url('enrol_trainee/edit',$call->id)}}" target="_blank"><span class="las la-edit" style="font-size: 30px; color: #3f4046;"></span></a>
        ...
        <div class="popup" id="popup-1">
            ...
            <div class="content">
                ...
                <h4 class="h44">اضافة   متدرب جديد للصف الدراسي </h4>

                </div>
                <!-- <div class="containerr"> -->
                <form id="myForm" action="{{url('enrol_trainee/store')}}" method="post" enctype="multipart/form-data" style="padding: 20px;color: black;">
                @csrf
                      <div class="roww">


                        <div class="input-groupp input-groupp-icon">
                        <input type="hidden" name="bimar_course_enrollment_id" value="{{ $course_id }}">                          @error('bimar_course_enrollment_id')
        ...
        <div class="popup" id="popuppo-1">
         ...
         <div class="gf">
                ...
                <h4 class="h44">  نقل المتدرب الى صف اخر </h4>
...
@else
    <p>لا توجد فصول متاحة لنقل المتدرب إليها</p>
@endif
                </div>
    ...
    let url = "{{ url('enrol_trainee/store') }}"; // URL الخاص بالـ POST
   ...
   function showEditPopup(id) {
    fetch(`/enrol_trainee/edit/${id}`)
        .then(response => response.json())
        .then(data => {
            console.log('بيانات المتدرب:', data);

            // تعيين بيانات الفصل المختار داخل القائمة المنسدلة
            let selectElement = document.querySelector('select[name="bimar_enrol_class_id"]');
            if (selectElement) {
                selectElement.value = data.bimar_enrol_class_id;
            }
...
function updateRole(event) {
    ...
    let url = `/enrol_trainee/update/${id}`;

    fetch(url, {
        method: 'POST',  // استخدم POST بدلاً من PUT
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
        },
        body: formData
