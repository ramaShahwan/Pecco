...
<div class="row" style="    margin: 80px 30px; direction: rtl;">
    ...
    color: white;">
                            ...
                            @foreach($data as $call)
                                <tr class="ttr">
                                <td>{{$i++}}</td>
                                    <td>{{$call->tr_users_status_name_ar}}  </td>
                                    <td>{{$call->tr_users_status_name_en}}</td>
                                    <!-- <td><label class="switch">

    <input type="checkbox" class="switch-button" data-id="{{ $call->tr_type_status }}" {{ $call->tr_type_status == 1 ? 'checked' : '' }}>
    <span class="slider"></span>
</label></td> -->
<td>   <a href="status/{{$call->id}}" class="btn btn-sm btn-{{$call->tr_users_status ? 'success' : 'danger'}}">
    {{$call->tr_users_status ? 'فعالة' : 'غير فعالة'}}
</a></td>
<td>{{$call->tr_users_status_desc}}</td>
                                    <td>
        ...
        <div class="popup" id="popup-1">
            ...
            <div class="content">
                ...
                <h4 class="h44">اضافة حالة مستخدم جديدة </h4>

                </div>
                <!-- <div class="containerr"> -->
                <form id="myForm" action="{{url('status/store')}}" method="post" enctype="multipart/form-data" style="padding: 20px;color: black;">
                @csrf
                      <div class="roww">
        ...
        <div class="popup" id="popuppo-1">
         ...
         <div class="gf">
                ...
                <h4 class="h44">   تعديل حالة المستخدم</h4>
         ...
         <input type="hidden" name="id" value="{{ $call->id }}">
            <div class="roww">
                ...
                <div class="input-groupp input-groupp-icon">
                    <div class="input-icon"><i class="fa-solid fa-signature"></i></div>
                    <input type="text" id="tr_users_status_name_ar" name="tr_users_status_name_ar" placeholder="الاسم باللغة العربية" value="{{ $call->tr_users_status_name_ar }}" class="@error('tr_users_status_name_ar') is-invalid @enderror"/>
                    @error('tr_users_status_name_ar')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
                ...
                <div class="input-groupp input-groupp-icon">
                    <input type="text" id="tr_users_status_name_en" name="tr_users_status_name_en" placeholder="الاسم باللغة الانكليزية" value="{{ $call->tr_users_status_name_en }}" class="@error('tr_users_status_name_en') is-invalid @enderror"/>
            ...
            <div class="roww">
                ...
                <div class="input-groupp input-groupp-icon">
                    <input type="text" name="tr_users_status_desc" id="tr_users_status_desc" placeholder="الوصف" value="{{ $call->tr_users_status_desc }}" />
                    <div class="input-icon"><i class="fa-solid fa-audio-description"></i></div>
                </div>
            </div>
    ...
    let url = "{{ url('status/store') }}"; // URL الخاص بالـ POST
    ...
    fetch(`/status/edit/${id}`)
        .then(response => response.json())
        .then(data => {
            console.log('Data received:', data);

            // Assign the values to the correct fields
            document.getElementById('tr_users_status_name_ar').value = data.tr_users_status_name_ar; // Arabic name
            document.getElementById('tr_users_status_name_en').value = data.tr_users_status_name_en; // English name
            document.getElementById('tr_users_status_desc').value = data.tr_users_status_desc;
...
function updateStatus(event) {
    ...
    const data = {
        ...
        tr_users_status_name_en: document.getElementById('tr_users_status_name_en').value,
        tr_users_status_name_ar: document.getElementById('tr_users_status_name_ar').value,
        tr_users_status_desc: document.getElementById('tr_users_status_desc').value,

        tr_users_status: document.querySelector('input[name="tr_users_status"]:checked').value,
        id: document.querySelector('input[name="id"]').value
    };

    let url = `/status/update/${data.id}`;
