...
<td>   <a href=" updateSwitch/{{$call->id}}" class="btn btn-sm btn-{{$call->tr_pay_status ? 'success' : 'danger'}}">
    {{$call->tr_pay_status ? 'فعالة' : 'غير فعالة'}}
</a></td>
    ...
    fetch(`/pay_status/edit/${id}`)
        ...
        .then(data => {
            ...
            document.getElementById('tr_pay_status_desc').value = data.tr_pay_status_desc; // Arabic name

            // Update the radio button for type status
            document.querySelector(`input[name="tr_pay_status"][value="${data.tr_pay_status}"]`).checked = true;
...
function updatePay_status(event) {
    ...
    const data = {
        ...
        tr_pay_status_name_ar: document.getElementById('tr_pay_status_name_ar').value,
